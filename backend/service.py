...
class service:
    ...
    def __init__(self, databaseURL):
        self.dataURL = databaseURL
        d = DatabaseSingleton(databaseURL)
        app.run()
        print("service")
    ...
    def preise(self):
        ...
        if filter == "durchschnitt":
            return json.dumps(self.durchschnittsWerte(begin, end))
        elif filter == "id":
            return json.dumps(self.getTankstellenPreis(interval, begin, end, id))
        elif filter == "all":
            return json.dumps(self.getTankstellenPreis(interval, begin, end))
    ...
    def tankstellen(self):
        filter = request.args.get('filter', default="all", type=str)
        # cursor = connection.cursor()

        # Zeigt alle Tankstellen an
        if filter == "all":
            cursor = self.d.getCursor()
    ...
    def getTankstellenPreis(self, interval="days", begin=datetime.now().strftime("%Y-%m-%d 00:00:00"),
                            end=datetime.now().strftime("%Y-%m-%d 23:23:59"), id=""):
        query = ""

        avg = self.durchschnittsWerte(begin, end)
